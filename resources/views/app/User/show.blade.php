...
@section('content')
            ...
            <div class="card-body">
                <form action="{{ route('updated-user', ['uuid' => $user->uuid]) }}" method="POST" class="row">
                    ...
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 d-flex justify-content-center align-items-center flex-column text-center mb-3">
                        <img src="{{ Auth::user()->avatar ? asset('storage/'.Auth::user()->avatar) : asset('assets/img/avatars/man.png') }}" alt="Perfil de {{ Auth::user()->name }}" class="d-block w-px-100 h-px-100 rounded-4" id="change-avatar-button" style="cursor: pointer;"/>
                    </div>
                    ...
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12 mb-3">
                        <div class="form-floating form-floating-outline">
                            <input type="text" name="bio" id="bio" class="form-control" placeholder="CEP:" value="{{ $user->bio }}"/>
                    ...
                    <div class="col-6 col-sm-12 col-md-6 offset-md-6 col-lg-6 offset-lg-6 mb-3 d-flex justify-content-end align-items-center">
                        <button type="submit" class="btn btn-outline-dark w-100 mt-2">Salvar</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <form action="{{ route('updated-user', ['uuid' => $user->uuid]) }}" method="POST" enctype="multipart/form-data" id="photo-upload-form" class="d-none">
        ...
        <input type="file" name="avatar" id="avatar-input" accept="image/*" onchange="document.getElementById('avatar-upload-form').submit();">
    </form>
